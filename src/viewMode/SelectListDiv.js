        ...
        if (this.props.inputs != null) {
            for (let i = 0; i < this.props.inputs.length; i++) {
                ...
                if (this.props.name == 'episodes' && this.props.episodeNumber != null) {
                    let episodeNumber = 'S' + this.state.seasonNumber + ':E' + (i + 1);
                    episodeNumberSpan = <span className='trackNumber'>{episodeNumber}</span>
                ...
                if (this.props.selectedValue == this.props.inputs[i]) {
                    ...
                    items.push(<div className='listItem selected'>
                        {episodeNumberSpan}<span className='trackTitle'>{this.props.inputs[i]}</span></div>);
                }
                else {
                    items.push(<div className='listItem' onClick={this.changeSelection.bind(this, this.props.inputs[i])}>
                        {episodeNumberSpan}<span className='trackTitle'>{this.props.inputs[i]}</span></div >);
            ...
            if (this.state.episodeNumber != null) {
                ...
                if (this.props.episodeTitle != null) {
                    ...
                    if (this.props.episodeNumber == null) {
                        ...
                        episodeNumber = <span className='episodeNumber'>{this.props.episodeNumber}</span>
                        let episodeNumberSplit = this.props.episodeNumber.split(":");
                        console.log(episodeNumberSplit);
                        if (episodeNumberSplit.length > 1 && this.state.seasonNumber != episodeNumberSplit[0].substring(1)) {

                            this.setState({
                                seasonNumber: episodeNumberSplit[0].substring(1)
                            });
                        }
